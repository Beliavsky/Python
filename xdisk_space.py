...
DIR_RE = re.compile(r"^\s*Directory of\s+(.+?)\s*$", re.IGNORECASE)
FILES_RE = re.compile(r"^\s*([\d,]+)\s+File\(s\)\s+([\d,]+)\s+bytes\s*$", re.IGNORECASE)
...
def bucket_name(rel_dir: str, max_depth: int) -> str:
    if max_depth == 0:
        return "."
...
def parse_dir_s_lines(lines, max_depth: int):
    root_dir = None
    current_dir = None
    ...
    for raw_line in lines:
        ...
        if "Total Files Listed" in line:
            in_total_section = True
            continue

        m_files = FILES_RE.match(line)
        if in_total_section or not m_files or current_dir is None or root_dir is None:
            continue

        current_lower = current_dir.lower()
        root_lower = root_dir.lower()
        if max_depth > 0 and current_lower == root_lower:
            continue

        root_prefix = root_lower + "\\"
        if not current_lower.startswith(root_prefix):
            continue

        if current_lower == root_lower:
            rel = ""
        else:
            rel = current_dir[len(root_dir) + 1 :]
        directory_name = bucket_name(rel, max_depth)
        if not directory_name:
            continue
        file_count = int(m_files.group(1).replace(",", ""))
        size_bytes = int(m_files.group(2).replace(",", ""))
...
def parse_dir_s_output_file(input_file: str, max_depth: int):
    with open(input_file, "r", encoding="utf-8", errors="replace") as f:
        return parse_dir_s_lines(f, max_depth=max_depth)
...
def parse_dir_s_from_directory(directory: str, max_depth: int):
    ...
    for walk_root, _, files in os.walk(root, topdown=True, followlinks=False):
        ...
        if max_depth > 0 and not rel_str:
            continue
        directory_name = bucket_name(rel_str, max_depth)
        if not directory_name:
            continue
